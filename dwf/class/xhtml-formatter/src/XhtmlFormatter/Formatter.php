...
class Formatter {
    ...
    public function format(string $string): string {
        ...
        $this->outputIndentationUnit = $this->outputIndentationMethod === self::TABS_INDENTATION ? "\t" : str_repeat(' ', $this->outputIndentationSize);
        ...
        $tokens = preg_split(
                self::PREG_SPLIT_RE, $string, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY
        );
    ...
    private function decreaseIndentation() {
        $this->outputIndentation = preg_replace(
                        "/" . $this->outputIndentationUnit . "/", '', $this->outputIndentation, 1
                ) ?? '';
    }
    ...
    private function matchTokenType(string $token): string {
        ...
        if ($this->matchOpenTag($token, $matches)) {
            $type = in_array($matches['element'], $this->unpairedElements[$this->contentType]) ? self::TOKEN_UNPAIRED_TAG : self::TOKEN_OPEN_TAG;
    ...
    private function setCodePlaceholders(string $string): string {
        foreach ($this->codePlaceholdersRegularExpressions as $codePlaceholderRe) {
            ...
            foreach ($matches as $match) {
                ...
                $string = preg_replace(
                        '/' . preg_quote($fullMatch, '/') . '/', $fullMatchReplacement, $string, 1);
