...
class new_app {
    ...
    private function create_dir() {
        ...
        /* index */
        $index = '<?php class website { /** * Liste des classes metier et classes du framework * @var array */ public static $_class; /** * point de départ du site web */ public function __construct() { $this->classloader(); include_once "../../dwf/index.php"; try { new index(); } catch (Exception $e) { dwf_exception::print_exception($e); } } /** * Inclut toutes les classes du dossier "class" se finissant par ".class.php" <br /> * Vous pouvez créer vos propres classes avec cette extension pour les charger automatiquement avant de les utiliser dans votre application */ private function classloader() { foreach (glob(__DIR__ . "/class/*.class.php") as $class) { include_once $class; self::$_class[] = strtr($class, array(__DIR__ . "/class/" => "", ".class.php" => "")); } foreach (glob(__DIR__ . "/class/entity/*.class.php") as $class) { include_once $class; } } } new website();';
        ...
        if (isset($_POST["srv"])) {
            ...
            $file_services = '<?php class index_service { /** * Cette classe est la premiÃ¨re appelÃ©e, elle ouvre les variables de session et la connexion a la base de donnÃ©e, <br /> * redÃ©finie la time zone et fait appel Ã  ces mÃ©thodes privÃ©es avant d\'appeler la class application (IDEM __construct()...) */ public function __construct() { try { $this->classloader(); include "../class/config.class.php"; if (isset($_REQUEST["service"])) { date_default_timezone_set("Europe/Paris"); $this->entityloader(); application::$_bdd = new bdd(); session::start(false); $this->serviceloader(); $this->security_purge(); $service = strtr($_REQUEST["service"], array("." => "", "/" => "", "\\\\" => "", "?" => "", "#" => "")); if (file_exists($service . ".service.php")) { $service = new $service(); } else { dwf_exception::throw_exception(622, array("_s_" => $service)); } } else { dwf_exception::throw_exception(621); } } catch (Exception $e) { dwf_exception::print_exception($e, "", true); } } /** * Inclut toutes les classes du framework */ private function classloader() { foreach (glob("../../../dwf/class/*.class.php") as $class) { include_once $class; } } /** * Inclut les entitÃ©s du projet */ private function entityloader() { foreach (glob("../class/entity/*.class.php") as $class) { include_once $class; } } /** * Inclut toutes les classes du dossier "service" se finissant par ".service.php" */ private function serviceloader() { foreach (glob("*.service.php") as $class) { include_once $class; } } /** * Supprime tous les fichiers se terminant par .php~ (trill) dans le dossier "service" <br /> * Cette fonction est recommandÃ©e sur les serveurs de production Linux pour des raisons de sÃ©curitÃ© <br /> * certains hÃ©bergeurs tolÃ¨rent mal cette fonction, elle peut Ãªtre dÃ©sactivÃ©e en commentant la ligne "$this->security_purge();" dans le constructeur. */ private function security_purge() { foreach (glob("*.php~") as $trill) { unlink($trill); } }}new index_service();';
            mkdir($dir_services);
