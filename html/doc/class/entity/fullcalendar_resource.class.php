...
class fullcalendar_resource {
    ...
    public function __construct($data) {
        ...
        $this->set_eventColor($data["eventColor"]);
        $this->_this_was_modified = false;
    }

    /** Ajoute une entrée en base de donnée */
    public static function ajout($title, $eventColor) {
        $title = application::$_bdd->protect_var($title);
        $eventColor = application::$_bdd->protect_var($eventColor);
        application::$_bdd->query("INSERT INTO fullcalendar_resource(title, eventColor) VALUES('" . $title . "', '" . $eventColor . "');");
    }

    /** Retourne la structure de l'entity au format json */
    public static function get_structure() {
        return json_decode('[["id","int",true],["title","string",false],["eventColor","string",false]]', true);
    }
    ...
    public static function get_collection($where = "") {
        $data = fullcalendar_resource::get_table_array($where);
        $col = false;
        foreach ($data as $entity) {
            if ($entity != FALSE) {
                $col[] = new fullcalendar_resource($entity);
            }
        } return $col;
    }
    ...
    public static function get_table_array($where = "") {
        $req = "select * from fullcalendar_resource";
        if (!empty($where)) {
            $req .= " where " . $where;
        } $req .= " ;";
    ...
    public static function get_count($where = "") {
        $req = "select count(*) as count from fullcalendar_resource";
        if (!empty($where)) {
            $req .= " where " . $where;
        } $req .= " ;";
    ...
    public static function get_from_id($id) {
        $data = application::$_bdd->fetch("select * from fullcalendar_resource where id = '" . application::$_bdd->protect_var($id) . "';");
        if (isset($data[0]) and $data[0] != FALSE) {
            return new fullcalendar_resource($data[0]);
        } else {
            return false;
        }
    }
    ...
    public static function get_json_object($where = "") {
        return json_encode(fullcalendar_resource::get_table_ordored_array($where));
    }

    /*     * Supprime l'entité */

    public static function delete_by_id($id) {
        application::$_bdd->query("delete from fullcalendar_resource where id='" . application::$_bdd->protect_var((int) $id) . "';");
    }
    ...
    public function get_eventColor() {
    ...
    public function __destruct() {
        if ($this->_this_was_modified and ! $this->_this_was_delete) {
            ...
            $eventColor = application::$_bdd->protect_var($this->get_eventColor());
            application::$_bdd->query("update fullcalendar_resource set id = '" . $id . "', title = '" . $title . "', eventColor = '" . $eventColor . "' where id = '" . $id . "';");
